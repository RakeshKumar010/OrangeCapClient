...
const JobTable = ({ searchValue }) => {
  ...
  const downloadExcal = () => {
    ...
    XLSX.writeFile(wb, "jobData.xlsx");
  };
  ...
  const handleDelete = () => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          const response = await fetch(baseUrl + "/select-job-delete", {
            method: "DELETE",
            headers: {
